...
class CodeExecutor:
    ...
    async def _execute_single(self, code: str, lang: Language, msg: Message = None) -> ExecutionResult:
        start_time = time()
        ...
        with pyinstrument.Profiler():
            try:
                ...
                EXECUTION_TIME.labels(language=lang.value).observe(time() - start_time)
    ...
    @ray.remote
    def _execute_distributed(self, code: str, lang: str, msg: Message = None) -> ExecutionResult:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            return loop.run_until_complete(self._execute_single(code, Language[lang.upper()], msg))
        finally:
            loop.close()
    ...
    async def execute_batch(self, snippets: List[CodeSnippet], msg: Message = None) -> List[ExecutionResult]:
        ...
        if os.getenv("USE_RAY", "0") == "1":
            futures = [self._execute_distributed.remote(self, s.code, s.language.value, msg) for s in snippets]
