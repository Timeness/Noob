...
class ControllerBot:
    ...
    async def restrict_owner(func: Callable) -> Callable:
        ...
        async def wrapper(self, client, message):
            self.logger.info(f"Checking user ID: {message.from_user.id} against owner ID: {self.owner_id}")
            if message.from_user.id != self.owner_id:
                await message.reply("Unauthorized access.")
                return
            self.logger.info(f"Calling wrapped function: {func.__name__}")
            return await func(self, client, message)  # Await the coroutine
        return wrapper

    async def start_command(self, client: Client, message) -> None:
        self.logger.info("Start command triggered")
    ...
    async def about_command(self, client: Client, message) -> None:
        self.logger.info("About command triggered")
    ...
    async def test_command(self, client: Client, message) -> None:
        self.logger.info("Test command triggered")
        await message.reply("Test command works! Bot is receiving commands.")

    @restrict_owner
    async def clone_bot(self, client: Client, message) -> None:
        self.logger.info(f"Clone command received: {message.text}")
        ...
        try:
            ...
            self.logger.info(f"Bot cloned with token: {bot_token[:10]}...")
    ...
    async def stop_bot(self, client: Client, message) -> None:
        self.logger.info(f"Stop command received: {message.text}")
        args = message.text.split(maxsplit=1)
        if len(args) < 2:
        ...
        try:
            ...
            self.logger.info(f"Bot stopped: {bot_token[:10]}...")
